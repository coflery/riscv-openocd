...
typedef struct {
	...
	hpm_stat_t (*post_erase_sector_nonblocking)(XPI_TYPE *base, xpi_xfer_channel_t chn,
												xpi_nor_config_t *nor_cfg, uint32_t addr);

	 /**< Post Erase Block Nonblocking operation: For Hybrid mode only */
	hpm_stat_t (*post_erase_block_nonblocking)(XPI_TYPE *base, xpi_xfer_channel_t chn,
												xpi_nor_config_t *nor_cfg, uint32_t addr);
