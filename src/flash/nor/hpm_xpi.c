									...
									target_addr_t algo_entry)
{
	...
	int retval = target_run_algorithm(target, 0, NULL, 6, reg_params,
			algo_entry, algo_entry + FLASH_INIT + 4, 500, NULL);
	if (retval != ERROR_OK) {
...
static int hpm_xpi_probe(struct flash_bank *bank)
{
	...
	flash_info.total_sz_in_bytes = le_to_h_u32((uint8_t *)&flash_info.total_sz_in_bytes);
	flash_info.sector_sz_in_bytes = le_to_h_u32((uint8_t *)&flash_info.sector_sz_in_bytes);
...
static int hpm_xpi_get_info(struct flash_bank *bank,
	...
	uint32_t offset, uint32_t count)
{
	...
	int xlen = riscv_xlen(target);
	retval = target_read_memory(target, bank->base + offset,
						xlen >> 3, count / (xlen >> 3), buf_on_target);
	if (retval != ERROR_OK)
		return retval;
